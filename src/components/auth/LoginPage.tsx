...
export default function LoginPage() {
    ...
    return (
        <div className="relative min-h-screen w-full flex items-center justify-center overflow-hidden bg-slate-800 text-white">
            {/* Background com Ripple (Modo Escuro 3D) */}
            <div className="absolute inset-0 z-0 bg-slate-800 overflow-hidden flex items-center justify-center">
                <Ripple mainCircleSize={650} mainCircleOpacity={0.6} numCircles={14} />
            </div>

            {/* Card de Login Glassmorphism */}
            <div className="relative z-10 w-full max-w-md mx-4 bg-zinc-900/40 backdrop-blur-2xl rounded-2xl shadow-2xl shadow-blue-900/30">
                
                {/* Borda estática fina */}
                <div className="absolute inset-0 rounded-2xl border border-white/5 pointer-events-none" />
                
                {/* BorderBeam ajustado milimetricamente para ficar exatamente na quina e borda externa */}
                <div className="absolute -inset-[1px] rounded-2xl pointer-events-none">
                    <BorderBeam size={200} duration={8} colorFrom="#22d3ee" colorTo="#3b82f6" borderWidth={2} className="rounded-2xl" />
                </div>
